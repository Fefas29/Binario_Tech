...
curl -s -X POST http://localhost:3000/api/v1/alertas \
  ...
  -d '{
    ...
    "metadados": {
      ...
      "motorista": "Myguel dusse"
    }
  }' | jq .

echo -e "\n[2] Consultando Coleção Inteira de Alertas..."
curl -s http://localhost:3000/api/v1/alertas | jq .

echo -e "\n[3] Exercício 1: Consultando apenas Alertas por Severidade (CRITICO)..."
curl -s http://localhost:3000/api/v1/alertas/severidade/CRITICO | jq .
