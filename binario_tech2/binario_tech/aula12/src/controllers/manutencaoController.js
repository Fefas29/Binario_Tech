...
const manutencaoController = {
  // 1. Criar Registro com Subdocumentos
  criar: async (req, res) => {
    try {
  ...
  // 2. Listar com Filtro Avançado ($gte / status)
  listarComFiltros: async (req, res) => {
    try {
      ...
      res.status(500).json({ erro: "Erro ao consultar manutenções.", detalhe: erro.message });
    }
  },

  // 3. Buscar por Placa com Busca Parcial Case-Insensitive ($regex)
  ...
  // 4. Adicionar Peça (subdocumento) via $push
  ...
  excluir: async (req, res) => {
    try {
      ...
      res.status(500).json({ erro: "Erro ao excluir registro.", detalhe: erro.message });
    }
  }
};
